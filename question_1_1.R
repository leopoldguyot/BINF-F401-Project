...
numeric_clinical_data <- apply(
    clinical_data[-c(1:2, 13:14)],
    2,
    as.numeric
)
...
clinical_data_scale <- data.frame(
    clinical_data$SMPLID, clinical_data$SUBJID,
...
clinical_data_racine_caree <- data.frame(
    clinical_data$SMPLID, clinical_data$SUBJID,
    clinical_data$COHORT, clinical_data$SMPTHNTS,
    clinical_data$SMPLID.1, clinical_data$IMGURL,
    clinical_data$SEX, clinical_data$DTHHRDY,
...
clinical_data_Inverse_Normal_Transformation <- clinical_data[c(
    "SMPLID",
    "SUBJID",
    "COHORT",
    "SMPTHNTS",
...
INT_WGHT <- qnorm((rank(clinical_data$WGHT, na.last = "keep") - 0.375) / (sum(!is.na(clinical_data$WGHT)) - 2 * 0.375 + 1))
